...
- **Lighthouse's "efficient cache lifetimes" finding is unresolved — do not
  ...
  **2026-08-21: re-litigated and closed as an accepted limitation, not
  reopened.** Two more angles were considered and rejected before deciding
  to stop spending time on this:
  1. *Same proxy, via `next.config.ts` `rewrites()` instead of a hand-written
     route.* No different in practice — `opennextjs-cloudflare`'s
     `proxyExternalRequest` option (which governs how an external-`destination`
     rewrite is executed) defaults to `"node"`, meaning the Worker itself
     still makes the fetch and streams the response back, same as the
     hand-written route above. A rewrite-based version would hit the same
     unconfirmed resource-limit failure as attempt 3 above, just declared
     differently. And the dashboard Cache Rule step this approach depends on
     is the exact thing already confirmed dead below (next bullet) — a
     Worker-constructed response bypasses zone-level Cache Rules regardless
     of whether the Worker code was handwritten or generated by a rewrite.
     Making it actually cache would still mean the Worker itself calling
     `caches.default`, i.e. reintroducing outage #1's code path.
  2. *Defeat the Lighthouse audit itself* — defer the hero videos' load
     (e.g. behind a `fetch()` + artificial delay) so they load outside
     Lighthouse's capture window, rather than fixing the caching. Rejected on
     two grounds: Lighthouse records network activity through its
     network-idle wait, not just up to the `load` event, so a short delay
     likely wouldn't even outrun capture; and even if it did, it doesn't
     change what real visitors transfer on every repeat visit — it only stops
     measuring the problem, not fixing it.

  Decision: given the constraint of not adding external media storage
  (Cloudinary/S3/R2 — would change the CMS upload flow itself, see below)
  and not reopening Worker-response code without Workers Logs access, this
  finding is accepted as a permanent, documented limitation of Tina Cloud's
  asset CDN rather than something to keep re-attempting.
  **If circumstances change later** (Workers Logs access is obtained, or
  external media storage becomes acceptable), the two live options are: (a)
  redo the same-origin proxy with real stack traces and off-production
  load-testing this time, or (b) migrate off repo-based media to an S3-API
  provider (e.g. Cloudflare R2 with a custom domain, so it's a genuine
  cached-origin behind Cloudflare rather than a Worker-constructed response)
  — TinaCMS's `next-tinacms-s3` package officially supports AWS S3 only (not
  R2 by name, though R2's S3-compatible API should work with it
  unofficially), and its `createMediaHandler` is written for Pages Router
  `NextApiRequest`/`NextApiResponse`-style API routes, not App Router's Fetch
  API `route.ts` handlers — this repo is App Router only, so it'd need a
  coexisting `pages/api/...` route rather than dropping in as-is. That
  migration is separately a real workflow change, not just an infra swap:
  repo-based media uploads currently land as git commits (revertable,
  blamable); an external store's uploads write directly to the bucket with
  no git trail, and switching is all-or-nothing across every collection
  (Tina doesn't allow repo-based and external media configured at once).
